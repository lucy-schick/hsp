...
Field safety plans are a critical component of our work. They are a way to ensure that we are prepared for the unexpected and that we have a plan in place to keep our team safe. The field safety plan should be specific to each project and should be kept easily accessible to all team members. An example can be found [here](https://www.newgraphenvironment.com/safety_plan_skeena_2023/). Information included in these plans include:
  ...
  4. Personal and crew equipment checklist. A list of all required safety equipment can be found [here](https://github.com/NewGraphEnvironment/onboarding/blob/main/data/equipment_contacts_safetylog.xlsx). 
...
```{r equipment}
#Load equipment data 
equipment <- readxl::read_excel(path = "../onboarding/data/equipment_contacts_safetylog.xlsx", sheet = "equipment") %>%
  janitor::clean_names()
```

```{r personal-eq, caption="Personal Safety Equipment"}
equipment |> 
  filter(!is.na(eq_pers_standard)) |> 
  select(eq_item) |> 
  rename (Items = eq_item) |>
  fpr::fpr_kable()
```

<br>

```{r truck-eq, caption="Truck Safety Equipment"}
equipment |> 
  filter(!is.na(eq_truck)) |> 
  select(eq_item) |>
  rename (Items = eq_item) |>
  fpr::fpr_kable()
```
